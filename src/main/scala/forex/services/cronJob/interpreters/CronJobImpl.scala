...
class CronJobLive[F[_]: Concurrent: Timer](config: OneFrameConfig,
                                           ratesService: RatesService[F],
                                           cacheService: CacheService[F])
    extends CronJob[F] {

  override def job(): fs2.Stream[F, (Either[Error, Unit], FiniteDuration)] =
    Stream(()).repeat
      .evalMap(_ => {
        for {
...
object CronJobLive {
  def apply[F[_]: Concurrent: Timer](config: OneFrameConfig,
                                     ratesService: RatesService[F],
                                     cacheService: CacheService[F]): CronJobLive[F] =
    new CronJobLive(config, ratesService, cacheService)
}
